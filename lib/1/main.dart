...
class _MyHomePage extends State<MyHomePage> {
  ...
  void DateNow() {
    ...
    String date = outputFormat.format(_now);
  }
...
class _NextPageState extends State<NextPage> {
  ...
  //登録した日（食品を追加した日）
  List<Refri> _memoList2 = [];
  Stream<int> initializeDemo2() async* {
    _memoList2 = await Refri.getDate();
  }

  //保存期間の基準
  List<Material_db> _memolist3 = [];
  Stream<int> initializeDemo3() async* {
    _memolist3 = await Material_db.getExday();
    //widgettitle: compinfo.needLandscapingHelp;
  }
  ...
  */
  ...
  String formatNow = formatter.format(_now);*/
  //賞味期限計算
  void expiry() {
    final a = Refri.getDate();
    final b = Material_db.getExday();
    //var today1=[];
    //var _today = DateFormat('y/M/d').parse('a'); //
    //.add(Durationの前に変数を使うとエラーになる
    var expiry_date = DateFormat("yyyy/MM/dd")
        .parse('a')
        .add(Duration(days: int.parse('b'))); //
  }
  //var expiry_date = DateTime(today1.day + int.parse('Material_db.getExday'));
  //var expiry_date;
  /*final prefs = await SharedPreferences.getInstance();
  prefs.setString('count', count);*/
                    ...
                    children: <Widget>[
                      ...
                      SizedBox(
                        child: ElevatedButton(
                            child: Text('button'), onPressed: DateNow),
                      ),
                      ...
                      SizedBox(
                        child: ElevatedButton(
                            child: Text('button'), onPressed: expiry),
                      ),
                      /*SizedBox(
                        child: ElevatedButton(
                          child: Text('button'),
